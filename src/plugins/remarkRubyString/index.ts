...
const remarkRubyStringDirective: Plugin<[], Root> = () => {
  return (tree) => {
    visit(tree, isTextDirective, (node) => {
      if (node.name !== "rubyString") return;
        ...
        hChildren: [
          {
            type: "element",
            tagName: "span",
            properties: {style:"color: #ff0000;font-weight: bold;"},
            children: [
              {
                type: "text",
                value: value,
...
export default remarkRubyStringDirective;